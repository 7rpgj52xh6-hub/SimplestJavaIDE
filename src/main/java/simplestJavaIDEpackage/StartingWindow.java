	...
	private void initialize() {
		...
		btnOpenExistingCode.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				FileNameExtensionFilter filter = new FileNameExtensionFilter("SimplestJavaIDE Files", "sji");
				...
				if (fileChooser.showOpenDialog(frmSimplestJavaIDE_startingWindow) == JFileChooser.APPROVE_OPTION) {
					if (fileChooser.getSelectedFile().getAbsolutePath().contains(" ")) {
						...
						ErrorPopupWindow.throwMessage("File path can't contain any period/dot. Please reselect.");
					} else {
						if (fileChooser.getSelectedFile().exists()) {
							File file1 = fileChooser.getSelectedFile();
							CodingFile codingFile = FileManager.load(file1.getAbsolutePath(), false);
			...
			public void actionPerformed(ActionEvent e) {
				...
				if (fileChooser.showSaveDialog(frmSimplestJavaIDE_startingWindow) == JFileChooser.APPROVE_OPTION) {
					if (fileChooser.getSelectedFile().getAbsolutePath().contains(" ")) {
						...
						ErrorPopupWindow.throwMessage("File name can't start with a number. Please reselect.");
					} else if (fileChooser.getSelectedFile().getName().replaceFirst(fileFormat, "").contains(".")) {
						ErrorPopupWindow.throwMessage("File path can't contain any period/dot. Please reselect.");
					} else {
						...
						if (!file.exists()) {
							if (!file.toString().endsWith(fileFormat)) {
								file = new File(file.toString() + fileFormat);
							}
							try {
								file.createNewFile();
								CodingFile codingFile = FileManager.load(file.getAbsolutePath(), true);
								MainUserInput.main(null, codingFile);
								frmSimplestJavaIDE_startingWindow.dispose();
							} catch (IOException e1) {
								e1.printStackTrace();
